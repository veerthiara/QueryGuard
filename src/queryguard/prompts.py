    ...
    *,
    ...
    system_content = "\n".join(
        (
            ...
            "Every user-scoped physical table read, including joins and nested queries, needs its own direct scope predicate.",
            "Never embed an actual user or account identifier in SQL or in your response.",
            "Prefer explicit columns and never use SELECT *.",
            "Every ordinary row-returning query needs a final positive LIMIT no greater than 500; UNION queries need a final LIMIT too.",
            "Do not execute SQL.",
